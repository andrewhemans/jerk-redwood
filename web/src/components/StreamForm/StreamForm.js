...
const StreamForm = (props) => {
  const onSubmit = (data) => {
    props.onSave(data, props?.stream?.id)
  }
  ...
  return (
    <div className="box-border text-sm -mt-4">
      <Form onSubmit={onSubmit} error={props.error}>
        ...
        <TextField
          name="body"
          defaultValue={props.stream?.body}
          className={CSS.input}
          errorClassName={CSS.inputError}
          validation={{ required: true }}
        />
        ...
        <TextField
          name="when"
          defaultValue={props.stream?.when}
          className={CSS.input}
          errorClassName={CSS.inputError}
          validation={{ required: true }}
        />
        <FieldError name="when" className={CSS.errorMessage} />
        ...
        <TextField
          name="link"
          defaultValue={props.stream?.link}
          className={CSS.input}
          errorClassName={CSS.inputError}
          validation={{ required: true }}
        />
        <FieldError name="link" className={CSS.errorMessage} />
        ...
        <TextField
          name="authorId"
          defaultValue={props.stream?.authorId}
          className={CSS.input}
          errorClassName={CSS.inputError}
          validation={{ required: true }}
        />
